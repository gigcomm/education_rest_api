...
def test_list_tasks_returns_pydantic_models(
    ...
    assert result == [
        TaskReadShema(id="task-1", title="Изучить pytest", completed=False),
        TaskReadShema(id="task-2", title="Написать первый тест", completed=True),
    ]
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            TaskUpdateResponse(title="Обновить заголовок"),  # payload
            "Обновить заголовок",  # expected_title
            False,  # expected_completed
        ),
        pytest.param(
            TaskUpdateResponse(completed=True),  # payload
            "Старая задача",  # expected_title
            True,  # expected_completed
        ),
        pytest.param(
            TaskUpdateResponse(title="Готово", completed=True),  # payload
...
def test_update_task_raises_when_task_not_found(
    ...
    with pytest.raises(TaskNotFound):  # Должна произойти указанная ошибка
        service.update_task("missing-task", TaskUpdateResponse(title="Неважно"))
